...
class SplashActivity : BaseAppCompatActivity() {
    ...
    private fun initUnpackItems() {
        ...
        //内置整合包：与 Java 环境一起在启动画面解压
        //放在这里可以让「铺资源」这件事集中在一处完成，
        //进主界面后不需要再等一次安装。
        val builtinTask = UnpackBuiltinModpackTask(this@SplashActivity)
        if (builtinTask.checkState() != InstallableItem.State.NOT_EXISTS) {
            unpackItems.add(
                InstallableItem(
                    "山之城",
                    "内置整合包",
    ...
    private fun handleBuiltinModpack(): Boolean {
        ...
        return try {
            if (BuiltinDirectInstaller.isInstalled()) {
                ...
                Logger.info(TAG, "启动画面未能解压内置整合包，退回主界面安装流程")
